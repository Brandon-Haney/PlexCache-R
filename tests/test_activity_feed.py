...
class TestFileActivity:
    ...
    def test_all_fields_present(self):
        ...
        with patch('web.services.operation_runner.get_time_format', return_value='24h'):
            fa = FileActivity(
                ...
                size_bytes=100,
                users=["bob"],
                associated_files=[{"filename": "test.nfo", "size": "9 B"}],
            )
            d = fa.to_dict()

        required_keys = {'timestamp', 'time_display', 'date_key', 'date_display', 'action', 'filename', 'size', 'users', 'associated_files'}
        assert required_keys == set(d.keys())

    def test_to_dict_includes_associated_files(self):
        """to_dict() includes associated_files when present."""
        with patch('web.services.operation_runner.get_time_format', return_value='24h'):
            fa = FileActivity(
                timestamp=datetime.now(),
                action="Cached",
                ...
                size_bytes=1000,
                associated_files=[
                    {"filename": "movie.nfo", "size": "1.00 KB"},
                    {"filename": "movie.srt", "size": "50.00 KB"},
                ],
            )
            d = fa.to_dict()

        assert d['associated_files'] == [
            {"filename": "movie.nfo", "size": "1.00 KB"},
            {"filename": "movie.srt", "size": "50.00 KB"},
        ]

    def test_to_dict_omits_associated_files_when_empty(self):
        """to_dict() omits associated_files when the list is empty."""
        with patch('web.services.operation_runner.get_time_format', return_value='24h'):
            fa = FileActivity(
                timestamp=datetime.now(),
                action="Cached",
    ...
    def test_backward_compat_no_associated_files(self):
        """FileActivity defaults to empty associated_files list."""
        fa = FileActivity(
            timestamp=datetime.now(),
            action="Cached",
            filename="movie.mkv",
        )
        assert fa.associated_files == []
...
class TestSaveActivity:
    ...
    def test_round_trip_with_associated_files(self, tmp_path):
        """Save then load preserves associated_files field."""
        f = tmp_path / "activity.json"
        now = datetime.now()
        original = [
            FileActivity(
        ...
        assert len(loaded) == 1
        assert loaded[0].associated_files == [
            {"filename": "movie.nfo", "size": "9 B"},
            {"filename": "movie.srt", "size": "50.00 KB"},
        ]
...
# ============================================================================
# TestSiblingActivityMerge
# ============================================================================

class TestSiblingActivityMerge:
    """Tests for OperationRunner._merge_sibling_activities()."""

    def _make_runner(self, activities, activity_file):
        """Create an OperationRunner with pre-loaded activities on disk."""
        with patch('web.services.operation_runner.load_activity', return_value=list(activities)):
            runner = OperationRunner()

        # Save to disk so _merge_sibling_activities can load-merge-save
        with patch('web.services.operation_runner.ACTIVITY_FILE', activity_file), \
    ...
    def test_basic_merge(self, tmp_path):
        """NFO sibling folds into parent video activity."""
        f = tmp_path / "activity.json"
        now = datetime.now()
        activities = [
            FileActivity(timestamp=now, action="Cached", filename="Movie.mkv", size_bytes=1_000_000),
            FileActivity(timestamp=now, action="Cached", filename="Movie.nfo", size_bytes=9),
        ]
        runner = self._make_runner(activities, f)

        sibling_map = {
            "/mnt/user/Movies/Movie/Movie.mkv": ["/mnt/user/Movies/Movie/Movie.nfo"],
        }
        ...
        assert len(result[0].associated_files) == 1
        assert result[0].associated_files[0]["filename"] == "Movie.nfo"

    def test_empty_sibling_map_noop(self, tmp_path):
        """Empty sibling map does not modify activities."""
        f = tmp_path / "activity.json"
        now = datetime.now()
        activities = [
            FileActivity(timestamp=now, action="Cached", filename="Movie.mkv", size_bytes=1000),
        ]
        runner = self._make_runner(activities, f)

        with patch('web.services.operation_runner.ACTIVITY_FILE', f), \
             patch('web.services.operation_runner._get_activity_retention_hours', return_value=9999):
            runner._merge_sibling_activities({})
        ...
        assert len(result) == 1
        assert result[0].associated_files == []

    def test_no_matching_parent_stays_standalone(self, tmp_path):
        """Sibling without matching parent activity stays as its own row."""
        f = tmp_path / "activity.json"
        now = datetime.now()
        activities = [
            FileActivity(timestamp=now, action="Cached", filename="Other.mkv", size_bytes=1000),
            FileActivity(timestamp=now, action="Cached", filename="Movie.nfo", size_bytes=9),
        ]
        runner = self._make_runner(activities, f)

        # Parent "Movie.mkv" is NOT in activities
        sibling_map = {
            "/mnt/user/Movies/Movie/Movie.mkv": ["/mnt/user/Movies/Movie/Movie.nfo"],
        }
    ...
    def test_ambiguous_sibling_not_merged(self, tmp_path):
        """Sibling basename mapped to multiple parents stays standalone."""
        f = tmp_path / "activity.json"
        now = datetime.now()
        activities = [
            FileActivity(timestamp=now, action="Cached", filename="MovieA.mkv", size_bytes=1000),
            FileActivity(timestamp=now, action="Cached", filename="MovieB.mkv", size_bytes=2000),
            FileActivity(timestamp=now, action="Cached", filename="poster.jpg", size_bytes=50),
        ]
        runner = self._make_runner(activities, f)

        # poster.jpg appears under two different parent videos
        sibling_map = {
            "/mnt/user/Movies/A/MovieA.mkv": ["/mnt/user/Movies/A/poster.jpg"],
            "/mnt/user/Movies/B/MovieB.mkv": ["/mnt/user/Movies/B/poster.jpg"],
        }
        ...
        # All 3 should remain (poster.jpg not merged due to ambiguity)
        assert len(result) == 3

    def test_multiple_siblings_merged(self, tmp_path):
        """Multiple sibling types (NFO, SRT, poster) all fold into parent."""
        f = tmp_path / "activity.json"
        now = datetime.now()
        activities = [
            FileActivity(timestamp=now, action="Cached", filename="Movie.mkv", size_bytes=1_000_000),
            FileActivity(timestamp=now, action="Cached", filename="Movie.nfo", size_bytes=9),
            FileActivity(timestamp=now, action="Cached", filename="Movie.srt", size_bytes=50_000),
            FileActivity(timestamp=now, action="Cached", filename="poster.jpg", size_bytes=200_000),
        ]
        runner = self._make_runner(activities, f)
        ...
        sibling_map = {
            "/mnt/user/Movies/Movie/Movie.mkv": [
                ...
                "/mnt/user/Movies/Movie/Movie.srt",
        ...
        assert len(result) == 1
        assert result[0].filename == "Movie.mkv"
        assert len(result[0].associated_files) == 3
    ...
    def test_different_action_not_merged(self, tmp_path):
        ...
        activities = [
            FileActivity(timestamp=now, action="Cached", filename="Movie.mkv", size_bytes=1_000_000),
            FileActivity(timestamp=now, action="Restored", filename="Movie.nfo", size_bytes=9),
        ]
        runner = self._make_runner(activities, f)
